...
One type of operation typically used in "tidying" nested lists is the `spread` operation.  In the `tidytree` package, this is called `spread_list`.  `spread_list` uses a `col_spec` object to spread nested list "keys" out into new columns.  Some of the main features:
